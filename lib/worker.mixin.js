...
module.exports = {
    ...
    methods: {
        ...
        async handle({message,stream,id}) {
            this.logger.debug("Missing message handler", { stream: stream });
            return false;
        },
        ...
        async consume () {
            ...
            try {
                let elements = await this.broker.call(this.actions.read, params, { meta: meta });
                    ...
                    await Promise.all(elements.map(async (element) => {
                        ...
                        try {
                            ...
                            if (result) {
                                ...
                                await this.broker.call(this.actions.ack, params, { meta: meta });
                            }
                        } catch (err) {
                ...
                this.logger.error("Error reading stream", { action: this.actions.read, params: params, error: err });
            }
    ...
    async created() { 
        ...
        this.stream = _.get(this.settings,"stream",null);
        if (!this.stream) {
        ...
        this.group = _.get(this.settings,"group",null);
        if (!this.group) {
            this.logger.error("Missing setting for consumer group");
            throw new Error("Consumer group setting is missing.");
        }
        this.actions = {
            read: _.get(this.settings,"actions.read","streams.read"),
            ack: _.get(this.settings,"actions.ack","streams.ack")
        };
